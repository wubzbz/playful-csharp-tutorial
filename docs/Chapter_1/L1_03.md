...
除了和一个常量比较相等性以外，比较大于、小于等其他关系也是很常用的。这种模式叫做**关系模式**。
...
关系模式下的switch语句工作原理和常量模式很类似，`case >= 95`实际上是在检测条件`positivePercent >= 95`。如果匹配上了，就执行`Console.WriteLine("好评如潮");`。然后，`break;`标志着switch语句的结束，程序会执行后续代码。如果没匹配上，那就说明好评率是不足95%的，那么程序会继续尝试匹配下一个case，也就是`case >= 80`。
