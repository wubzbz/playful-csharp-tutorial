...
请看上例。第3行有两个单等号`=`，它们都是赋值运算符，形成了连续赋值，这点我们[之前](./L1_02.md/#_5)说过了。重点应该在最后一行。我们从右往左看：`a == b`有双等号，是在判断`a`和`b`是否相等。这里`a`和`b`都是5，因此结果是相等（`true`），这个结果被提供给布尔变量`isEqual`。至此，你能分清`=`和`==`的区别了吗？
...
其中`VipTime`表示会员资格时长，而且只有会员用户才有这个变量。那就有问题了——如果不是会员用户，也许根本就不存在什么`VipTime`了。此时尝试计算右边的表达式必然引发类似 当前上下文中不存在名称“VipUser” 的错误。
...
!!! info
    ...
    有没有不管左边结果如何都计算右边条件的运算符呢？

    诶，还真有！它们就是`&`和`|`。上面说的`&&`和`||`运算符的优点反过来就是`&`和`|`的缺点。此外还有一个异或运算符`^`，它的功能和`!=`一样。
...
那么，现在的未读消息数量是4，尝试运行代码，应该会输出"There's a red point at the icon."。然后，如果把`newMessageNum`设置成0，再次运行代码，会发现没有任何输出。看上去很不错！

在这里，条件`showRedPoint`只使用了一次，所以我们也可以不单独声明一个变量来储存它：
...
- 如果第一个if的条件（红灯）是真的，就执行对应的代码块（停），然后**跳过**后续所有和这个if搭配的else if以及else语句。
- 如果第一个if的条件（红灯）是假的，那就跳到下一个else if的条件判断（绿灯）。
...
也就是说，1个`lightStatus`聚合了`isRedLight`, `isYellowLight`, `isGreenLight`这3个功能相似的变量，这个过程叫提高**内聚度**。

我们从前面使用诸如`isRedLight && !isYellowLight && !isGreenLight`这样的条件表达式判断3个变量之间的（耦合）关系，到只需判断`lightStatus`这1个变量的数值，这个过程叫降低**耦合度**。
...
在讲[贯穿](#_6)的末尾，测验时间提到“合并”的case之间是逻辑**或**的关系。C#提供了一个关键词`or`，用来帮助我们把两个或关系的case真正地合并到一起：
